...
class Level2_Screen(QWidget):
    ...
    def draw_sin_wave(self):
        ...
        prev_point = QPoint(0, mid_y)
        for x in range(1, width):
            y = mid_y - int(math.sin(x * 0.005) * amplitude)
            curr_point = QPoint(x, y)
            painter.drawLine(prev_point, curr_point)
            prev_point = curr_point
    def handle_next(self):
    ...
    def save_image_and_log(self):
        ...
        row = ws.max_row + 1
        ws.cell(row=row, column=1).value = self.player_name
        ws.cell(row=row, column=3).value = "Level 2"
        try:
            thumbnail_img = Image.open(processed_path)
            thumbnail_img.thumbnail((150, 75), Image.LANCZOS)
            thumbnail_img.save(excel_img_path)
    ...
    def tabletEvent(self, event: QTabletEvent):
        if event.type() == QTabletEvent.TabletPress:
            ...
            self.last_point = event.pos()
            self.update()
            print(f"PenInput: x={event.pos().x()}, y={event.pos().y()}, pressure={pressure:.3f}")
        elif event.type() == QTabletEvent.TabletRelease:
            self.last_point = None
        event.accept()
...
class Level3_Screen(QWidget):
    def __init__(self, main_dialog, player_name, cursor_pixmap):
        super().__init__()
        self.setWindowTitle("Level 3 - Pen Input")
        self.setMinimumSize(2000, 1100)
        self.setMouseTracking(True)
        ...
        self.label = QLabel(f"Level 3 - Welcome {self.player_name}")
        self.label.setFont(QFont("Arial", 28))
        self.label.setStyleSheet("color: white;")
        ...
        # Canvas for drawing
        self.drawing = QPixmap(2000, 1100)  # safer default size
        self.drawing.fill(Qt.blue)

        # Trail image
        self.trail_img = QPixmap("C6.png")
        if self.trail_img.isNull():
            print("ERROR: Could not load C6.png from", os.getcwd())
    ...
    def tabletEvent(self, event: QTabletEvent):
        if event.type() == QTabletEvent.TabletPress:
            self.last_point = event.pos()
        elif event.type() == QTabletEvent.TabletMove and self.last_point:
            painter = QPainter(self.drawing)
            pen = QPen(Qt.darkRed, event.pressure() * 50)
            painter.setPen(pen)
            painter.drawLine(self.last_point, event.pos())
            self.last_point = event.pos()
            self.update()
    ...
    def update_trail(self):
        ...
        self.trail_pos += QPoint(int(dx * 0.1), int(dy * 0.1))
        self.update()
...
class Level4_Screen(QWidget):
    def __init__(self, main_dialog, player_name, cursor_pixmap):
        super().__init__()
        self.setWindowTitle("Level 2- Pen Input")
        self.setMinimumSize(2000, 1100)
        self.setMouseTracking(True)
...
class Level5_Screen(QWidget):
    def __init__(self, main_dialog, player_name, cursor_pixmap):
        ...
        self.sin_points = self.draw_sin_wave()


    def draw_sin_wave(self):
        painter = QPainter(self.drawing)
        font = QFont("Lucida Handwriting", 100, QFont.Bold)  # Try Lucida Handwriting if preferred
...
class Level6_Screen(QWidget):
    def __init__(self, main_dialog, player_name, cursor_pixmap):
        ...
        if self.original_bg_pixmap.isNull():
            print("Error: Could not load snail_back.png")
            return
    ...
    def paintEvent(self, event):
        base_painter = QPainter(self)
        base_painter.drawPixmap(300, 200, self.original_bg_pixmap.scaled(1900,900, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        base_painter.drawPixmap(0, 0, self.drawing)
